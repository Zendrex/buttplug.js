...
Target multiple features simultaneously by including multiple tracks. Each track operates independently on its assigned `featureIndex`. The cycle duration is the longest track; shorter tracks hold their final value until the cycle ends.
...
Set `outputType` explicitly to target a specific output type when a device has mixed feature types. Valid values are listed in [`OUTPUT_TYPES`](/docs/client/constants#output_types). By default, the engine resolves tracks to compatible features automatically.
...
Custom tracks do not loop by default. Pass `loop: true` for infinite repetition or a number for a fixed cycle count. The `intensity` option scales keyframe values: `value: 1` at `intensity: 0.5` becomes `0.5`.
...
Looping with [lifecycle callbacks](/docs/patterns/examples). [`Track` and `Keyframe` types](/docs/patterns/types).
