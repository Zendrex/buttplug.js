...
<Callout type="warn">
`onComplete` fires **before** `onStop`. When a pattern finishes on its own, both run in order: `onComplete`, then `onStop` with reason `"complete"`.
</Callout>

## Dynamic Pattern Switching

A new `play()` on a device stops any existing pattern on that device without a prior `stop()`.
...
[`PatternEngine` reference](/docs/patterns/pattern-engine).
