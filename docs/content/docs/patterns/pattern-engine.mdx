...
```ts
new PatternEngine(client: PatternClient, options?: { defaultTimeout?: number })
```

| Parameter | Type | Description |
| --- | --- | --- |
| `client` | `PatternClient` | Client providing device access and event hooks |
...
The engine subscribes to client `connection.disconnected` and `device.removed` events. When the client disconnects, all patterns are stopped with reason `"disconnect"`. When a device is removed, its patterns are stopped with reason `"deviceRemoved"`.
...
| Parameter | Type | Description |
| --- | --- | --- |
| `target` | `PatternDevice \| number` | Target device instance or device index |
...
The `reason` parameter is a [`StopReason`](/docs/patterns/types#stopreason) string.
...
- [Types](/docs/patterns/types): PatternDescriptor, Track, Keyframe, and more
