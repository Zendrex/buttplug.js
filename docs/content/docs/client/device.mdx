...
The `Device` class provides typed methods for controlling outputs (vibration, rotation, position, etc.) and reading or subscribing to input sensors. Device instances are created internally by [ButtplugClient](/docs/client/buttplug-client) when devices are discovered.
...
Parsed input and output feature descriptors for this device. See [DeviceFeatures](/docs/client/types#devicefeatures) for the shape.
...
All scalar output methods accept either a single `number` (applied to all motors/actuators) or a `FeatureValue[]` array for per-feature control. **All values are normalized 0–1 floats**; the library maps each value to the feature's device range internally.
...
Sets vibration intensity on all or individual motors. `intensity` is a normalized **0–1** float.

| Parameter | Type | Description |
| --- | --- | --- |
| `intensity` | `number \| FeatureValue[]` | Single value (0–1) for all motors, or per-motor entries |
...
Speed is a normalized **0–1** float.

```ts
await device.rotate(0.7, { clockwise: false });

await device.rotate([
  { index: 0, speed: 0.8, clockwise: true },
  { index: 1, speed: 0.4, clockwise: false },
]);
```
...
Position is a normalized **0–1** float; `duration` is in milliseconds.
...
Sends a raw protocol output command to a specific feature. **Unlike the typed methods, `output()` uses device-range integers** (the raw protocol values), not normalized 0–1 floats. Values are clamped to the feature's declared range.
...
- [Types](/docs/client/types): FeatureValue, RotationValue, PositionValue, and more
