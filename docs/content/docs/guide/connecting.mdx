...
In application code, prefer **subscribing to client events** (for example `deviceAdded`, `disconnected`, `error`) and handling **typed errors** over relying on internal logs. See [Debugging and diagnostics](#debugging-and-diagnostics) below for when to use browser DevTools or an optional logger.
...
| `verbose` | `boolean` | `false` | When `true` and `logger` is omitted, enables prefixed `console` diagnostics (`consoleLogger`) |
| `logger` | `Logger` | `noopLogger` | Optional. Injectable diagnostics; overrides `verbose`. Prefer events for app behavior |
...
client.on("reconnecting", ({ data: { attempt } }) => {
...
1. **Events and errors** — Use `client.on("error", …)` and other [client events](/docs/reference/types#clienteventmap) for user-visible behavior. Thrown and emitted errors use the `ButtplugError` hierarchy (see the README **Error Handling** section and exported error types).

2. **WebSocket frames** — In Chromium-based browsers, open DevTools → **Network**, select the Buttplug WebSocket, then **Messages**. This is the most direct way to verify what the client and server actually exchanged (handshake, device list, commands).

3. **Optional diagnostics** — Set [`verbose`](/docs/reference/buttplug-client#buttplugclientoptions) when `logger` is omitted, or inject [`consoleLogger`](/docs/reference/types#logger) / a custom [`Logger`](/docs/reference/types#logger). For code that constructs components outside [`ButtplugClient`](/docs/reference/buttplug-client), [`resolveDiagnosticsLogger`](/docs/reference/types#resolvediagnosticslogger) applies the same `logger`/`verbose` resolution. Production builds usually leave both off (`noopLogger` by default).
