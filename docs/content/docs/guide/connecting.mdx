...
| `reconnectDelay` | `number` | -- | Initial delay (ms) before first reconnection attempt |
| `maxReconnectDelay` | `number` | -- | Maximum delay (ms) between attempts (exponential backoff cap) |
| `maxReconnectAttempts` | `number` | -- | Give up after this many consecutive failures |
| `requestTimeout` | `number` | -- | Timeout (ms) for individual protocol requests |
