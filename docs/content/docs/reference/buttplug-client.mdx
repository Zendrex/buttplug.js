...
description: High-level client for communicating with a Buttplug server over WebSocket or a custom transport.
---
...
For in-browser Web Bluetooth without Intiface Central, pass a [`WasmTransport`](/docs/reference/wasm-transport) instead of a URL. See the [WASM Transport](/docs/guide/wasm) guide.
...
| `target` | [`Transport`](/docs/reference/types#transport) | Custom transport (e.g. [`WasmTransport`](/docs/reference/wasm-transport)) |
...
client.on("disconnected", ({ data: { reason } }) => {
